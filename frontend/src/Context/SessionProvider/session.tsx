 ...
 * PProvides session context to the component tree, including managing subscribers, connections, and layout mode.
 ...
 */
const SessionProvider = ({ children }: { children: ReactNode }): ReactElement => {
  const session = useRef<Session | null>(null);
  const [reconnecting, setReconnecting] = useState(false);
  const [subscriberWrappers, setSubscribersWrapper] = useState<SubscriberWrapper[]>([]);
  ...
  const moveSubscriberToTopOfDisplayOrder = useCallback((id: string) => {
    setSubscribersWrapper((prevSubscriberWrappers) => {
    ...
    async (stream: Stream, localSession: Session, options: SubscriberProperties = {}) => {
      ...
      if (localSession) {
        ...
        subscriber.on('videoElementCreated', (event: VideoElementCreatedEvent) => {
          ...
          setSubscribersWrapper((previousSubscriberWrappers) =>
        ...
        subscriber.on('destroyed', () => {
          ...
          setSubscribersWrapper((prevSubscriberWrappers) =>
            prevSubscriberWrappers.filter(({ id }) => streamId !== id)
          );
        });
